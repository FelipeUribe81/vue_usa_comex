      ...
      <v-col cols="12" sm="6" md="12">
        ...
        >
          ...
          <p v-if="downloadTableAlert">
            Porfavor elija entre que páginas desea generar el reporte (Elija
            entre un máximo de 100 paginas)
            <v-row class="mt-4">
              <v-col cols="10">
                <v-range-slider
                  v-model="range"
              ...
              <v-col cols="2" class="mt-2">
                <v-btn icon :disabled="!validRangeToFetch">
                  <download-excel
...
export default {
  ...
  computed: {
    validRangeToFetch() {
      return this.range[1] - this.range[0] <= 1000;
    },
    rules() {
      return {
        max: (value) =>
          value >= this.min || `La cantidad de páginas es: ${this.max}.`,
        min: (value) =>
          value <= this.max || "No se permiten páginas negativas o nulas.",
      };
    },
  },
  methods: {
    ...
    async fetchData() {
      ...
      let tableParameters = {
        fecha_inicio: dates["start"],
        fecha_fin: dates["final"],
        init_range: this.range[0] * 10 - 10,
        end_range: this.range[1] * 10,
      };
      const response = await axios.post(
        `${this.url}/importacion/table/`,
        tableParameters,
        {
          headers: {
